...
class Pannellum extends PureComponent {
  ...
  renderImage = state => {
    ...
    if (Array.isArray(hotspots)) {
      // Filter out null/undefined hotspots to prevent errors
      hotspots = hotspots.filter(hotspot => hotspot !== null && hotspot !== undefined && hotspot.props);
      hotspots.map(hotspot => {
        // Additional safety check
        if (!hotspot || !hotspot.props) return null;
