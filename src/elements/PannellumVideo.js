...
class PannellumVideo extends Component {
  ...
  safePlay = async (videoPlayer) => {
    ...
    try {
      ...
      // This prevents infinite loops when browser pauses background media
      if (error.name === 'AbortError' || error.name === 'NotAllowedError') {
        // Browser paused video to save power or user interaction required
  ...
  renderVideo = (state) =>{
    ...
    if (state === "update"){
      // Prevent re-initializing if videojs is already initialized
      if (!this.video || !this.videoNode) {
        return;
      }
      
      // Check if videojs instance exists, if not create it
      if (!this.video.readyState) {
        this.video = videojs(this.videoNode);
      }
      
      // Only update if viewer exists
      if (this.video.pnlmViewer) {
      ...
      // Only update video source if it changed
      if (this.video.currentSrc() !== this.props.video) {
        this.video.src({ 
          type: 'video/mp4', 
      ...
      if (this.props.autoplay) {
        this.safePlay(this.video);
      }
      return;
    } else {
      // Initial mount - create videojs instance
      this.video = videojs(this.videoNode, {
